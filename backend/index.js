...
app.post('/api/vendas-mensais', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('vendas_mensais').upsert({
            quiosque_id: quiosque_id,
            ano: ano,
            mes: mes,
            valor_total: valor_total
        }, { onConflict: 'quiosque_id, ano, mes' }).select();
        if (error) throw error;
...
app.get('/api/relatorio-vendas', async (req, res) => {
    try {
        ...
        if (error) throw error;
        // ... (Lógica de KPIs e processamento de dados para o gráfico)
        const faturamentoPorAno = {}, faturamentoPorQuiosqueAno = {}, faturamentoPorMesAno = {};
        data.forEach(venda => {
            const ano = venda.ano.toString(), nomeQuiosque = venda.quiosques.nome;
        ...
        data.forEach(venda => {
            const nomeQuiosque = venda.quiosques.nome, mesAno = `${venda.ano}-${String(venda.mes).padStart(2, '0')}`;
...
app.get('/api/produtos/pesquisar', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('produtos').select('id, codigo_sku, descricao, preco_padrao').or(`descricao.ilike.%${query}%,codigo_sku.ilike.%${query}%`).limit(10);
        if (error) throw error;
        res.json(data);
    } catch (error) {
    ...
    try {
        ...
        Papa.parse(arquivoCsv, {
            ...
            complete: async (results) => {
                ...
                // O código agora procura por 'codigo_sku', 'descricao' e 'preco_padrao'
                ...
                if (produtosParaInserir.length === 0) {
                    return res.status(400).json({ message: "Nenhum produto válido foi encontrado no arquivo CSV. Verifique se as colunas 'codigo_sku', 'descricao' e 'preco_padrao' existem." });
                }
