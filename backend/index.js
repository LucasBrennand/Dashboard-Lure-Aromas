...
app.post('/api/produtos', async (req, res) => {
    ...
    try {
        const { data, error } = await supabase.from('produtos').insert([{ codigo_sku, descricao, preco_padrao }]).select().single();
        if (error) {
            if (error.code === '23505') {
                return res.status(409).json({ message: `O código SKU '${codigo_sku}' já existe.` });
            }
            throw error;
        }
...
app.put('/api/produtos/:id', async (req, res) => {
    ...
    try {
        ...
        if (error) {
            if (error.code === '23505') {
                return res.status(409).json({ message: `O código SKU '${codigo_sku}' já pertence a outro produto.` });
            }
            throw error;
        }
...
app.get('/api/produtos/exportar', async (req, res) => {
    try {
        const { data, error } = await supabase.from('produtos').select('codigo_sku, descricao, preco_padrao').order('descricao', { ascending: true });
        if (error) throw error;
