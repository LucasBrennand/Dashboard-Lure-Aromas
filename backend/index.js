...
const xlsx = require("xlsx"); // Importação para criar arquivos Excel
const multer = require("multer"); // Importação para upload de arquivos
const axios = require("axios"); // Importação para requisições HTTP
...
app.post("/api/vendas-mensais", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao salvar os dados no banco.", error });
  }
});
...
app.get("/api/relatorio-vendas", async (req, res) => {
  try {
    const { data, error } = await supabase.from("vendas_mensais").select(`
                ano,
                mes,
                valor_total,
                quiosques ( nome )
    ...
    data.forEach((venda) => {
      ...
      if (!faturamentoPorQuiosqueAno[ano][nomeQuiosque])
        faturamentoPorQuiosqueAno[ano][nomeQuiosque] = 0;
      if (!faturamentoPorMesAno[ano]) faturamentoPorMesAno[ano] = {};

      const mesNome = new Date(ano, venda.mes - 1).toLocaleString("pt-BR", {
        month: "long",
      });
      if (!faturamentoPorMesAno[ano][mesNome])
        faturamentoPorMesAno[ano][mesNome] = 0;
    ...
    for (const ano in faturamentoPorAno) {
      ...
      const quiosqueDestaque =
        Object.keys(quiosquesDoAno).length > 0
          ? Object.keys(quiosquesDoAno).reduce((a, b) =>
              quiosquesDoAno[a] > quiosquesDoAno[b] ? a : b
            )
          : "N/A";

      const mesesDoAno = faturamentoPorMesAno[ano];
      const melhorMes =
        Object.keys(mesesDoAno).length > 0
          ? Object.keys(mesesDoAno).reduce((a, b) =>
              mesesDoAno[a] > mesesDoAno[b] ? a : b
            )
          : "N/A";
      ...
      kpis[ano] = {
        ...
        quiosqueDestaque: {
          nome: quiosqueDestaque,
          valor: quiosquesDoAno[quiosqueDestaque] || 0,
        },
      };
    }
...
app.get("/api/produtos", async (req, res) => {
  ...
  try {
    ...
    if (search) {
      query = query.or(
        `descricao.ilike.%${search}%,codigo_sku.ilike.%${search}%`
      );
    }
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar produtos no banco de dados." });
  }
});
...
app.post("/api/produtos", async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("produtos")
      .insert([{ codigo_sku, descricao, preco_padrao }])
      .select()
      .single();
    if (error) {
      if (error.code === "23505") {
        return res
          .status(409)
          .json({ message: `O código SKU '${codigo_sku}' já existe.` });
      }
      throw error;
    }
    res
      .status(201)
      .json({ message: "Produto adicionado com sucesso!", product: data });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Erro ao salvar o produto.", error: error.message });
  }
});
...
app.put("/api/produtos/:id", async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from("produtos")
      .update({ codigo_sku, descricao, preco_padrao })
      .eq("id", id)
      .select()
      .single();
    if (error) {
      if (error.code === "23505") {
        return res
          .status(409)
          .json({
            message: `O código SKU '${codigo_sku}' já pertence a outro produto.`,
          });
      }
      throw error;
    }
...
/**
 * ROTA PARA EXPORTAR TODOS OS PRODUTOS PARA PLANILHA
 */
app.get("/api/produtos/exportar", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="catalogo_produtos.xlsx"'
    );
    res.setHeader(
      "Content-Type",
    ...
    res
      .status(500)
      .json({ message: "Ocorreu um erro ao gerar a planilha de produtos." });
  }
});

// --- ROTA DE ADMINISTRAÇÃO / CARGA INICIAL ---
...
app.post("/api/importar-produtos", async (req, res) => {
  ...
  try {
      ...
      complete: async (results) => {
        ...
        const produtosParaInserir = results.data
          ...
          .map((produto) => ({
            ...
            preco_padrao:
              parseFloat(String(produto.preco_padrao).replace(",", ".")) || 0,
          }));
        ...
        if (produtosParaInserir.length === 0) {
          return res
            .status(400)
            .json({
              message: "Nenhum produto válido foi encontrado no arquivo CSV.",
            });
        }

        console.log(
          `Encontrados ${produtosParaInserir.length} produtos para inserir.`
        );
        try {
          const { data, error } = await supabase
            .from("produtos")
          ...
          res
            .status(200)
            .json({
              message: `${data.length} produtos foram importados com sucesso!`,
            });
        } catch (dbError) {
          ...
          res
            .status(500)
            .json({
              message: "Erro ao inserir produtos no banco.",
    ...
    res
      .status(500)
      .json({
        message: "Não foi possível ler o arquivo produtos.csv no backend.",
      });
  }
});
